...
```text
initial state = SHA256( 0xC0DECAFE || 0(28バイト分) || 0(4バイト分) || salt || password )
```
3. 次にこの初期ハッシュを使って再ハッシュ化を100000回繰り返して簡易鍵伸長をします。
```text
for( uint32 i = 0...99999 )
    state = SHA256( state || i || salt || password )
```
4. 鍵伸長を終えた鍵をHMAC-SHA256で以下のようにハッシュ化してマスターハッシュを生成します。
```text
state = HMAC-SHA256( state, 0xC0DECAFE || 0 )
```
...
for( i = 0...filesize )
    if( i == 0 or j == 0 )
        keystream = HMAC-SHA256( state, 0x00C0FFEE || cnt )
    ...
    if( j == 0 )
        cnt   = cnt + 1
        state = HMAC-SHA256( state, 0xC0DECAFE || cnt )
```
